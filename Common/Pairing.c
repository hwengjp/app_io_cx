...
#define V_PRINTF(...) if( !gc_sPairingConf.bDisableUART ) TWE_fprintf( (gc_sPairingConf.psSer), __VA_ARGS__)	//!< VERBOSE モード時の printf 出力
 ...
 */
#define gc_sPairingConf sAppDataExt.In_Paring_C.sPairingConf //!< ペアリングの設定パラメタ構造体
#define gc_u8Port sAppDataExt.In_Paring_C.u8Port //!< ペアリングしているときのステータスを見るためのDIOポート番号 0x80,0x81:DO0,1を使用する。 0x00-0x14:DIO0-18(20)を使用する。 0xFF:使用しない
#define gc_bDOFlag sAppDataExt.In_Paring_C.bDOFlag //!< DOを使用するかどうかのフラグ
#define gc_u8RemoteConf sAppDataExt.In_Paring_C.u8RemoteConf //!< どんなパケットを送信したかを判別するためのフラグ
#define gc_u8Flags sAppDataExt.In_Paring_C.u8Flags //!< 何を受信したかを判別するためのフラグ
#define gc_u32RcvAddr sAppDataExt.In_Paring_C.u32RcvAddr //!< 受信した相手のアドレス
#define gc_sPairingInfo sAppDataExt.In_Paring_C.sPairingInfo //!< ペアリング結果を保存する構造体
#define gc_bLEDBlink sAppDataExt.In_Paring_C.bLEDBlink //!< 点滅フラグ
#define gc_au8DataParing sAppDataExt.In_Paring_C.au8Data //!< 汎用データ領域
#define gc_u8CountSWC sAppDataExt.In_Paring_C.u8CountSWC	//!< 送信回数 E_STATE_WAIT_COMMAND
...
PRSEV_HANDLER_DEF(E_STATE_IDLE, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	if (eEvent == E_EVENT_START_UP) {
		// 初期化
		memset( &gc_sPairingInfo, 0x00, sizeof(gc_sPairingInfo) );
		gc_sPairingInfo.au8Data = gc_au8DataParing;

		// LEDを使用しなければ点滅しない
		if( IS_NOUSE_LED(gc_u8Port) ){
			gc_bLEDBlink = FALSE;
		}else{
			//	出力ポートの初期化
			if( gc_bDOFlag ){
				vAHI_DoSetDataOut( gc_u8Port+1, 0 );
			}else{
				vPortSetHi( gc_u8Port );
				vPortAsOutput( gc_u8Port );
			}
		}
...
PRSEV_HANDLER_DEF(E_STATE_WAIT_COMMAND, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	...
	// 60ms 経過後はペアリング受信待ち
	if (PRSEV_u32TickFrNewState(pEv) > 60 || gc_u8Flags == 1 ) {
		if( gc_u8Flags == 1 ){		//	成功したのでAckを返す状態へ
			gc_u8Flags = 0xFF;
			ToCoNet_Event_SetState(pEv, E_STATE_RUNNING);
		}else{
			if( gc_u8CountSWC == gc_sPairingConf.u8Retry ){		//	規定回数リトライ下かどうかの判断
				// ペアリング要求待ち
...
PRSEV_HANDLER_DEF(E_STATE_APP_LISTEN, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	if(  gc_u8Flags == 0 ){
		// 設定パケット送信
		if ( !bTranmitRespond( gc_u32RcvAddr ) ) {
	...
	if ( PRSEV_u32TickFrNewState(pEv) > gc_sPairingConf.u32ListenWait_ms ) {
...
PRSEV_HANDLER_DEF(E_STATE_APP_COMP, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	if (eEvent == E_EVENT_NEW_STATE) {
		gc_bLEDBlink = FALSE;
		if( gc_u8Flags != 2 && gc_u8RemoteConf != 2 ){	//	Ack送信完了もAck受信もしていない場合失敗
			V_PRINTF( LB"!INF: PAIRING FAILURE");
			gc_sPairingInfo.u32DstAddr = 0xFFFF;
			gc_sPairingInfo.u32PairKey = 0x00000000;
			gc_sPairingInfo.bStatus = FALSE;		// 失敗
			//	LED点灯
			if( !IS_NOUSE_LED(gc_u8Port) ){
				//	出力ポートの初期化
				if( gc_bDOFlag ){
					vAHI_DoSetDataOut( 0, gc_u8Port+1 );
				}else{
					vPortSetLo( gc_u8Port );
				}
			}
		}else{		//	成功した場合
			gc_sPairingInfo.u32DstAddr = gc_u32RcvAddr;
			gc_sPairingInfo.bStatus = TRUE;		// 成功
			if( gc_u8Flags == 2 ){		//	最後にACKを受信した場合、ペアキーを自分のアドレスにする
				gc_sPairingInfo.u32PairKey = ToCoNet_u32GetSerial();
			}else{					//	最後にACKを送信した場合、ペアキーを相手のアドレスにする
				gc_sPairingInfo.u32PairKey = gc_u32RcvAddr;
			}
			//	LEDの消灯
			if( !IS_NOUSE_LED(gc_u8Port) ){
				//	出力ポートの初期化
			...
			V_PRINTF( LB"!INF: PAIRING SUCCESS -> %08X.", gc_sPairingInfo.u32PairKey);
		}
		//	コールバック関数で設定の保存等を行う。
		gc_sPairingConf.pf_cbSavePair( &gc_sPairingInfo );
	}
}
...
static void cbAppToCoNet_vHwEvent(uint32 u32DeviceId, uint32 u32ItemBitmap) {
	...
	case E_AHI_DEVICE_TICK_TIMER:
		if(gc_bLEDBlink){	//点滅フラグが立っているとき、128msごとにLEDを点滅させる
			if( (u32TickCount_ms >> 7)&1  ){
				bStateLED = TRUE;
			}
			if( gc_bDOFlag ){
				vAHI_DoSetDataOut( bStateLED ? 0 : gc_u8Port+1, bStateLED ? gc_u8Port+1 : 0 );
			}else{
				vPortSet_TrueAsLo(gc_u8Port, bStateLED);
			}
		}else{
...
static void cbAppToCoNet_vRxEvent(tsRxDataApp *pRx) {
	...
	if (pRx->u8Cmd == PAIR_PKTCMD) {
		...
		if (u32ver != gc_sPairingConf.u32AppVer ) {
			V_PRINTF( LB"!VERSION_U32");
			return;
		}

		// *   LQIの確認
		if (pRx->u8Lqi < gc_sPairingConf.u8ThLQI ) {
			V_PRINTF( LB"!LQI");
			return;
		}

		uint32 u32Key = G_BE_DWORD();
		if( gc_sPairingConf.u32PairKey != u32Key ){
			V_PRINTF( LB"!KEY");
			return;
		}
		...
		if( u8pkttyp < 2 ){
			gc_sPairingInfo.u8DataType = G_OCTET();
			gc_sPairingInfo.u8DataLength = G_OCTET();

			for( i=0; i<gc_sPairingInfo.u8DataLength; i++ ){
				gc_sPairingInfo.au8Data[i] = G_OCTET();
			}
		}

		//	受信完了
		gc_u32RcvAddr = pRx->u32SrcAddr;
		gc_u8Flags = u8pkttyp;
	}
}
 ...
 *
 */
static const tsCbHandlerPair sCbHandlerPair = {
 ...
 * アプリケーション初期化
 */
void MWPFX(vInitAppPairing)( tsPairingConf *psConfig ) {
	gc_psCbHandlerPair = &sCbHandlerPair;
	gc_pvProcessEvPair = vProcessEvCore;
	gc_sPairingConf = *psConfig;

	//	各要素の代入
	sToCoNet_AppContext.u32AppId = gc_sPairingConf.u32AppID;
	sToCoNet_AppContext.u8Channel = gc_sPairingConf.u8PairCh;
	sToCoNet_AppContext.u32ChMask = 1 << gc_sPairingConf.u8PairCh;

	//	特に指定がなければ待ち時間は5秒間
	if( gc_sPairingConf.u32ListenWait_ms == 0 ){
		gc_sPairingConf.u32ListenWait_ms = 5000;
	}

	if( gc_sPairingConf.u8LEDPort == PAIR_LED_NOUSE ){
		// LED出力を無効にする
		gc_u8Port = 0xFF;
	}else{
		// MSBが1なら DO(PWM2,3)を使用する
		gc_bDOFlag = (gc_sPairingConf.u8LEDPort&0x80) != 0x00 ? TRUE : FALSE;
		// 使用するポートの代入
		gc_u8Port = gc_sPairingConf.u8LEDPort&0x7F;
	}
	//	受信回路を開く
...
static bool_t bTranmitRequest()
{
	...
	//  *   OCTET(4) : アプリケーションバージョン
	S_BE_DWORD(gc_sPairingConf.u32AppVer);
	//  *   パケット種別 = 要求
	//  *   OCTET(4) : 同じアプリケーションかどうかを識別するための鍵
	S_BE_DWORD(gc_sPairingConf.u32PairKey);
	//  *   OCTET : データの識別子
	S_OCTET( gc_sPairingConf.u8DataType );
	//  *   OCTET : データ列の長さ
	S_OCTET( gc_sPairingConf.u8DataLength );
	//  *   OCTET(len) : データ
	uint8 i;
	for( i=0; i< gc_sPairingConf.u8DataLength; i++ ){
		S_OCTET( gc_sPairingConf.au8Data[i] );
	}
