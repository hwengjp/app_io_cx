 ...
 * 最大のIO数入力
 */
extern uint8 gc_u8_PORT_INPUT_COUNT;
/** @ingroup MASTER
 * 最大のIO数出力
 */
extern uint8 gc_u8_PORT_OUTPUT_COUNT;
...
extern const uint8 gc_au8PortTbl_DIn[MAX_IO_TBL];
extern uint8 gc_u8_PORT_INPUT_COUNT;
extern uint32 gc_u32_PORT_INPUT_MASK;
extern uint8 gc_au8PortTbl_DOut[MAX_IO_TBL];
extern uint8 gc_u8_PORT_OUTPUT_COUNT;
extern uint32 gc_u32_PORT_OUTPUT_MASK;
...
extern const uint8 MWPFX(au8IoModeTbl_To_LogicalID)[8]; //!< tePortConf2Mode から論理アドレスへの変換
 ...
 * チャネルマスクのプリセット
 */
extern const uint32 MWPFX(au32ChMask_Preset)[];
...
void vModbOut_AckNack(TWE_tsFILE *pSer, bool_t bAck);
void MWPFX(vModbOut_MySerial)(TWE_tsFILE *pSer);

bool_t bModbIn_Config(uint8 *p,  tsFlashApp *pConfig);
void vModbOut_Config(TWE_tsFILE *pSer, tsFlashApp *pConfig);

/*
 * 出力マクロ
 */
#define V_PRINT(...) if(TWEINTRCT_bIsVerbose()) TWE_fprintf(&gc_sSer,__VA_ARGS__) //!< VERBOSE モード時の printf 出力
#define S_PRINT(...) if(!TWEINTRCT_bIsVerbose()) TWE_fprintf(&gc_sSer,__VA_ARGS__) //!< 非VERBOSE モード時の printf 出力
#define V_PUTCHAR(c) if(TWEINTRCT_bIsVerbose()) TWE_fputc(c, &gc_sSer)  //!< VERBOSE モード時の putchar 出力
#define S_PUTCHAR(c) if(!TWEINTRCT_bIsVerbose()) TWE_fputc(c, &gc_sSer) //!< 非VERBOSE モード時の putchar 出力
#ifdef DEBUG_OUTPUT
#define DBGOUT(lv, ...) if(sAppData.u8DebugLevel >= lv) TWE_fprintf(&gc_sSer, __VA_ARGS__) //!< デバッグ出力
#else
